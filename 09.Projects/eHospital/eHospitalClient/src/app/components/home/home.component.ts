...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(this.auth.user.userType === "Doctor"){
      ...
      // Buraya hasta girişinde açılacak ekranı yapacağım.
    }
    else{
      this.getAllDoctors();
    }
  ...
  getAllDoctors() {
    this.http.get("https://localhost:7099/api/Appointments/GetAllDoctors").subscribe((res: any) => {
  ...
  getDoctorAppointments() {
    ...
    this.http.get(`https://localhost:7099/api/Appointments/GetAllByDoctorId?doctorId=${this.selectedDoctorId}`).subscribe((res: any) => {
  ...
  add(form:NgForm){
    if (form.valid) {
      ...
      this.http.post("https://localhost:7099/api/Appointments/Create", data).subscribe(res => {
  ...
  findPatientByIdentityNumber() {
    ...
    (`https://localhost:7099/api/Appointments/FindPatientByIdentityNumber`,
  ...
  onAppointmentDeleting(event: any) {
    ...
    if (result) {
      ...
      this.http.get(`https://localhost:7099/api/Appointments/DeleteById?id=${id}`).subscribe(res => {
        this.getDoctorAppointments();
      });
    }
  }
