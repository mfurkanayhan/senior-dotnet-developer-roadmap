    ...
    ) : IAuthService
{
    public async Task<Result<string>> ConfirmVerificationEmailAsync(int emailConfirmCode, CancellationToken cancellationToken)
    {
        ...
        if (user is null)
        {
            return Result<string>.Failure(500, "Email verification code is not available.");
        }
    ...
    public async Task<Result<string>> SendConfirmEmailAsync(string email, CancellationToken cancellationToken)
    {
        ...
        if (user.EmailConfirmed)
        {
            return Result<string>.Failure(500, "The user's email has already been verified.");            
        }
        ...
        {
            return Result<string>.Failure(500, "Verification mail is sent every 3 minutes.");
        }
...
";
    ...
    private string MaskEmail(string email)
    {
        ...
        if (atIndex == -1) return email; // If it is not a valid email address, return without making any changes
        ...
        var maskedUsername = username.Length > 1
            ...
            : username; // If the username is too short, it is not masked
        ...
        if (domainParts.Length > 1)
        {
            ...
            var maskedDomainName = domainName.Length > 2
                ...
                : domainName; // If the domain name is too short, it is not masked

            var maskedDomain = maskedDomainName + "." + string.Join(".", domainParts[1..]);
            return maskedUsername + "@" + maskedDomain;
        }

        return maskedUsername + "@" + domain; // If there is no dot in the email address or if it is unidentifiable
    }
}