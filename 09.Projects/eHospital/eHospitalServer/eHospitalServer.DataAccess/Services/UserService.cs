    ...
    IMapper mapper) : IUserService
{
    public async Task<Result<string>> CreateUserAsync(CreateUserDto request, CancellationToken cancellationToken)
    {
        ...
        if (request.Speciality is not null)
        {
            ...
            {
                Specialty = (Speciality)request.Speciality,
    ...
    public async Task<Result<List<User>>> GetAllPatientsAsync(CancellationToken cancellationToken)
    {
        var patients =
            await userManager
            .Users
            .Where(p => p.UserType == UserType.Patient)
            .OrderBy(p => p.FirstName)
            .ToListAsync(cancellationToken);

        return Result<List<User>>.Succeed(patients);
    }
}